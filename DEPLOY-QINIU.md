...
3. 选择可用的 Sandbox 区域和模板，核对计费并创建部署。本项目提交了本地验证过的 `deploy/static` 页面产物，七牛无需安装前端工具链；平台直接用基础模板自带的 Node 启动零第三方依赖服务器，监听 `0.0.0.0:3000` 并检查 `/`。
...
更新页面后先使用 Node.js 22.22.0 或更新版本运行 `npm ci` 和 `npm run prepare:qiniu`，再提交同步生成的 `deploy/static`。本地可用 `HOST=127.0.0.1 PORT=3000 node deploy/server.mjs` 验证七牛发布产物。
