...
3. 选择可用的 Sandbox 区域和模板，核对计费并创建部署。七牛 `base` 模板的系统 Node 版本较旧，且 npm 10 对跨平台可选二进制锁文件存在兼容问题，因此目标环境使用 `npm install --package-lock=false --no-audit --no-fund` 按 Linux 架构解析依赖并安装固定的 Node 22.22.0 运行时，再用该运行时构建和启动服务；仓库中的 lock 文件仍用于本地审计。服务监听 `0.0.0.0:3000`，平台检查 `/`。
